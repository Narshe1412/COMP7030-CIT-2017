...
public class myBinarySearchTreeImpl<T1 extends Comparable<? super T1>, T2> implements myBinarySearchTree<T1, T2> {
	...
	public int my_count_at_level(int level){
		...
		if (this.my_is_empty() == true) {
			num = 0; //if the tree is empty, no nodes are found
		} else {
			if(level == 1) { 
				num = 1; // if we're out of levels, then it means we reached the intended level, add 1
			} else { // reduce the current level by calling each subtree with -1 to the level
				int l1 = this.my_left_tree().my_count_at_level(level - 1);
				int l2 = this.my_right_tree().my_count_at_level(level - 1);
				num = l1 + l2; //sum the result of each subtree
			}
		}
	...
	public boolean my_is_balanced(){
		...
		if (this.my_is_empty() == true ) {
			return true; //if the tree is empty, then it's balanced
		} else {
			if(this.my_left_tree().my_length() - this.my_right_tree().my_length() > 1) { 
				return false; //if lenght of both subtrees is bigger than 1, it's not balanced
			} else if (this.my_left_tree().my_is_balanced() && this.my_right_tree().my_is_balanced()) {
				return true; // if both subtrees are balanced, then return that the tree is balanced
			} else {
				return false; // if they're not, return that the tree is not balanced
			}
		}
	}
	...
	public int my_count_smaller_nodes(T1 key){
		...
		if(this.my_is_empty() == true) {
			...
			int sum = this.my_root().getKey().compareTo(key) < 0 ? 1: 0; //add 1 if smaller, don't add anything if greater or equal
			int countleft = this.my_left_tree().my_count_smaller_nodes(key); 
			int countright = this.my_right_tree().my_count_smaller_nodes(key);
			return sum + countleft + countright;	// sum current comparison and the sum of each subtree
		}		
	}	
	...
	public int my_find_node_at_level(T1 key){
		...
		if(this.my_is_empty() == true) { //if the tree is empty, return 0
			return 0;
		} else if (this.my_root().getKey().compareTo(key) == 0) { //if we've found the node, return the level
			return 1;
		} else { //otherwise, go search left and right subtrees to find the level
			level = this.my_left_tree().my_find_node_at_level(key);
			if (level > 0) { //if the result of searching for a level was positive, add 1 to the recursion
				level ++;
				return level;
			} else { //otherwise try the right subtriee
				level = this.my_right_tree().my_find_node_at_level(key);
				if(level > 0) { //if the result was positive, add 1 to the recursion result
					level ++;
					return level;
				}
			}
		}
		return -1; //if the node was not found, return an invalid number (aka -1)
	}	
