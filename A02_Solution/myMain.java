...
public class myMain {
	...
	public static int getMaxValue(myStack<Integer> s){
		int res = 0; 
		myStack<Integer> newStack = stack_copy(s);

		res = extra01(newStack);

		return res;
	}

	private static int extra01(myStack<Integer> s) {
		int res = 0;
		int temp = 0;

		if(s.my_is_empty()) { //base case
			return temp;
		} else { //recursion
			temp = s.my_peek();
			s.my_pop();
			return Math.max(temp,extra01(s));
		}
		//return res;
	}
	...
	public static int extra02(myStack<Integer> s, int max){
		...
		if(s.my_is_empty()) {
			...
			temp = s.my_peek();
			s.my_pop();
			return extra02(s, Math.max(temp, max));
		}
		//return max;
	}
	...
	private static int extra03(myList<String> l, String word) {
		int count = 0;
		

		if (l.my_get_length() == 0) { //base case
			count = 0;
		} else { //recursion
			int index= l.my_get_length()-1;
			String element = l.my_get_element(index);
			l.my_remove_element(index);
			if (element.equals(word)) {
				count++;
			}
			count += extra03(l, word);
		}
		return count;
	}
	...
	public static int extra04(myList<String> l, String word, int index, int accum){
		...
		if (index <= 0) {
			...
			if (word.equals(l.my_get_element(index-1))) {
				accum ++;
			}
			index --;
			return extra04(l, word, index, accum);
		}
		//return accum;
	}	
	...
	public static int extra05(int n, int m) {
		int accum;

		if (m==0) {
			return 1;
		} else  {
			accum = n * extra05(n, m-1);
			return accum;
		}
	}
	...
	public static void draw_image(int num, int totalSize) {
		/*NOTE: definition for this method in the PDF gets totalSize
		as parameter to draw the picture, however in the examples you pass
		num as parameter to draw.
		We just need to substitute, down below in the code, the three references to
		num variable for totalSize and the definition will match the PDF 
		(although it will break the example)
		There's no need for two variables on this recursion. One is enough.
		*/
		if (num <= 1) {
